...
def analyze_cable_curl(keypoints):
    """
    Cable curl hareketini analiz eder
    ...
    if avg_angle >= 145:
        ...
        feedback = "Hareket başlangıç pozisyonu - Cable curl yapmaya hazır"
        is_correct = True
    
    # 30-45 derece - mükemmel curl pozisyonu
    elif 45 <= avg_angle <= 60:
        movement_state = "perfect_curl"
        feedback = "Mükemmel curl pozisyonu! Harika!"
        is_correct = True
    
    # 45 dereceden büyük - daha fazla bükmeli
    elif 65 < avg_angle < 115:
        movement_state = "need_more_curl"
        feedback = "Biraz daha kolunuzu bükün"
        is_correct = False
    
    # 30 dereceden küçük - çok fazla bükülmüş
    elif avg_angle < 35:
        movement_state = "over_curled"
        feedback = "Çok fazla büktünüz, biraz gevşetin"
        is_correct = False
...
def process_video(video_path, output_path=None):
    ...
    cv2.namedWindow("Cable Curl Analizi", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Cable Curl Analizi", 640, 480)
    ...
    while cap.isOpened():
        ...
        if results[0].keypoints is not None and len(results[0].keypoints.data) > 0:
            ...
            if len(keypoints) >= 11:  # Gerekli keypoints varsa
                ...
                feedback, is_correct, avg_angle, left_angle, right_angle, movement_state = analyze_cable_curl(keypoints)
                
                # Tur sayma mantığı
                if movement_state == "perfect_curl" and not completed_curl:
                    completed_curl = True
                ...
                cv2.putText(annotated_frame, f"Cable Curl Sayisi: {curl_count}", (10, 150), font, 0.7, (0, 255, 255), 2)
                cv2.putText(annotated_frame, f"Durum: {movement_state}", (10, 180), font, 0.6, (255, 255, 255), 2)
                
                # Hedef açı aralığını göster
                cv2.putText(annotated_frame, "Hedef: 45-60° (Curl), 145+° (Baslangic)", (10, height-20), font, 0.5, (255, 255, 255), 1)
                ...
                cv2.putText(annotated_frame, f"Cable Curl Sayisi: {curl_count}", (10, 60), font, 0.7, (0, 255, 255), 2)
        else:
        ...
        # Sonucu göster
        cv2.imshow("Cable Curl Analizi", annotated_frame)
    ...
    print(f"Toplam Cable Curl Sayisi: {curl_count}")

def main():
    # Video yolu
    video_path = "cable_curl.mp4"  # Kendi video dosyanızın yolunu yazın
