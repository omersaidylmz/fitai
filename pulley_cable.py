...
def analyze_pulley_cable(keypoints):
    """
    Pulley Cable hareketini analiz eder
    ...
    if 115 <= avg_angle <= 130:
        movement_state = "start_position"
        feedback = "Hareket başlangıç pozisyonu - Pulley Cable yapmaya hazır"
        is_correct = True
    
    # 30-45 derece - mükemmel curl pozisyonu
    elif 50 <= avg_angle <= 65:
        movement_state = "perfect_pulley_cable"
        feedback = "Mükemmel Pulley Cable pozisyonu! Harika!"
        is_correct = True
    
    # 45 dereceden büyük - daha fazla bükmeli
    elif 65 < avg_angle < 85:
        movement_state = "need_more_pulley_cable"
...
def process_video(video_path, output_path=None):
    ...
    cv2.namedWindow("Pulley Cable Analizi", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Pulley Cable Analizi", 640, 480)
    ...
    while cap.isOpened():
        ...
        if results[0].keypoints is not None and len(results[0].keypoints.data) > 0:
            ...
            if len(keypoints) >= 11:  # Gerekli keypoints varsa
                ...
                feedback, is_correct, avg_angle, left_angle, right_angle, movement_state = analyze_pulley_cable(keypoints)
                
                # Tur sayma mantığı
                if movement_state == "perfect_pulley_cable" and not completed_curl:
                    completed_curl = True
                ...
                cv2.putText(annotated_frame, "Hedef: 50-65° (Curl), 120+° (Baslangic)", (10, height-20), font, 0.5, (255, 255, 255), 1)
                ...
                cv2.putText(annotated_frame, f"Pulley Cable Sayisi: {curl_count}", (10, 60), font, 0.7, (0, 255, 255), 2)
        else:
