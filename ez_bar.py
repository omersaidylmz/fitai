...
def analyze_ez_bar(keypoints):
    """
    Ez Bar hareketini analiz eder
    ...
    if 150 <= avg_angle <= 160:
        movement_state = "start_position"
        feedback = "Hareket başlangıç pozisyonu - Ez bar yapmaya hazır"
        is_correct = True
    
    # 30-45 derece - mükemmel curl pozisyonu
    elif 30 <= avg_angle <= 40:
        movement_state = "perfect_ez_bar"
        feedback = "Mükemmel Ez bar pozisyonu! Harika!"
        is_correct = True
    
    # 45 dereceden büyük - daha fazla bükmeli
    elif 45 < avg_angle < 115:
        movement_state = "need_more_ez_bar"
        feedback = "Biraz daha kolunuzu bükün"
        is_correct = False
    
    # 30 dereceden küçük - çok fazla bükülmüş
    elif avg_angle < 20:
        movement_state = "over_bared"
...
def process_video(video_path, output_path=None):
    ...
    cv2.namedWindow("Ez bar Analizi", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Ez bar Analizi", 640, 480)
    ...
    while cap.isOpened():
        ...
        if results[0].keypoints is not None and len(results[0].keypoints.data) > 0:
            ...
            if len(keypoints) >= 11:  # Gerekli keypoints varsa
                ...
                feedback, is_correct, avg_angle, left_angle, right_angle, movement_state = analyze_ez_bar(keypoints)
                
                # Tur sayma mantığı
                if movement_state == "perfect_ez_bar" and not completed_curl:
                    completed_curl = True
                ...
                cv2.putText(annotated_frame, "Hedef: 30-40° (Curl), 155+° (Baslangic)", (10, height-20), font, 0.5, (255, 255, 255), 1)
                ...
                cv2.putText(annotated_frame, f"Ez Bar Sayisi: {curl_count}", (10, 60), font, 0.7, (0, 255, 255), 2)
        else:
        ...
        # Sonucu göster
        cv2.imshow("Ez Bar Analizi", annotated_frame)
    ...
    print(f"Toplam Ez Bar Sayisi: {curl_count}")

def main():
    # Video yolu
    video_path = "ez_bar.mp4"  # Kendi video dosyanızın yolunu yazın
